...
public class Reporter : MonoBehaviour
{
	...
	public int numOfLogsWarning = 0;
    //total number of errors
	public int numOfLogsError = 0;
    ...
    private GameObject _logiView;
    public bool Initialized = false;

    private void OnLevelLoaded(Scene scene, LoadSceneMode loadMode)
    {
        if (scene.name.Contains("Arena5V5_New")|| scene.name.Contains("UI_PVP_New"))
        {
            if (_logiView != null)
            {
                _logiView.SetActive(false);
            }
        }
        else
        {
    ...
    public void Initialize()
    {
        if (!created)
        {
            ...
            GameObject logView = Resources.Load<GameObject>("Prefabs/LogView");
            if (logView != null)
            {
                _logiView = Instantiate(logView);
                DontDestroyOnLoad(_logiView);
            }
        ...
        size.x = size.y = PlayerPrefs.GetFloat("Reporter_size", 28);
    ...
    void initializeStyle()
    {
        ...
        logScrollerSkin.verticalScrollbar.fixedWidth = size.x * 2;
        logScrollerSkin.horizontalScrollbar.fixedHeight = 0f;
        logScrollerSkin.verticalScrollbarThumb.fixedWidth = size.x * 2;
    ...
    void Start()
    {
        ...
        //Debugger.EnableLog = PlayerPrefs.GetInt("EnableLog", 0) != 0 ? true : false;
    ...
    void drawToolBar()
    {
        ...
        GUIStyle tmpStyle = (showLog) ? buttonActiveStyle : barStyle;
        if (GUILayout.Button(logContent, tmpStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
            showLog = !showLog;
            calculateCurrentLog();
        }
        tempRect = GUILayoutUtility.GetLastRect();
        GUI.Label(tempRect, logsText, lowerLeftFontStyle);
        /*
        ...
        tmpStyle = (showWarning) ? buttonActiveStyle : barStyle;
        if (GUILayout.Button(warningContent, tmpStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
            showWarning = !showWarning;
            calculateCurrentLog();
        }
        tempRect = GUILayoutUtility.GetLastRect();
        GUI.Label(tempRect, logsWarningText, lowerLeftFontStyle);
        /*
        ...
        tmpStyle = (showError) ? buttonActiveStyle : barStyle;
        if (GUILayout.Button(errorContent, tmpStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
            showError = !showError;
            calculateCurrentLog();
        }
        tempRect = GUILayoutUtility.GetLastRect();
        GUI.Label(tempRect, logsErrorText, lowerLeftFontStyle);
        /*
    ...
    void doShow()
    {
        ...
        /*
        if (gameObject.GetComponent<ReporterGUI>() == null)
        {
    ...
    void OnLevelWasLoaded()
    {
        ...
        currentScene = Application.loadedLevelName;
        // Debug.Log("Scene " + Application.loadedLevelName + " is loaded|Time=" + Time.time);
    }
